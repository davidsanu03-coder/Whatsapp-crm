  ...
  return (
    <div className="auth">
      <div className="authCard">
        ...
        <p>Sign in to manage your WhatsApp leads, conversations and follow-ups.</p>
        {sent ? (
          <div className="success"><strong>Check your email.</strong><br />We sent you a secure login link.</div>
        ) : (
          <form onSubmit={login}>
            <input type="email" placeholder="Your email address" value={email} onChange={(event) => setEmail(event.target.value)} required />
...
function Dashboard({ session }) {
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leads.filter((lead) => [lead.name, lead.email, lead.phone, lead.whatsapp_name, lead.business_name, lead.interest].filter(Boolean).some((value) => String(value).toLowerCase().includes(query)));
  }, [leads, search]);
  ...
  function openEmail(lead) {
    ...
    const subject = encodeURIComponent(`Following up with ${lead.name || "you"}`);
  ...
  return (
    <div className="app">
      <aside>
        ...
        <nav><b>Dashboard</b><span>Leads</span><span>Follow-ups</span><span>Messages</span></nav>
        <button className="logout" onClick={() => supabase.auth.signOut()}>Sign out</button>
      </aside>

      <main>
        <header><small>SALES PIPELINE</small><h1>Good to see you.</h1><p>{session.user.email}</p></header>

        <section className="stats">
          {[["New", "new"], ["Interested", "interested"], ["Proposals", "proposal"], ["Follow-ups", "follow_up"], ["Won", "won"]].map(([name, status]) => (
            <div key={status}><small>{name}</small><strong>{count(status)}</strong></div>
          ))}
        </section>
        ...
        <section className="panel">
          <div className="panelHead">
            <div><h2>Leads</h2><span>{leads.length} total leads</span></div>
            <button className="primaryButton" onClick={() => setShowAdd(true)}>+ Add lead</button>
          </div>
          <div className="toolbar"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, email, phone, business..." /></div>
              ...
              <div className="column" key={stage}>
                ...
                {filteredLeads.filter((lead) => lead.status === stage).map((lead) => (
                  <button className="lead" key={lead.id} onClick={() => setSelected(lead)}>
                    <strong>{lead.name || lead.whatsapp_name || lead.phone}</strong>
                    <small>{lead.email || "No email"}</small>
                    <small>{lead.business_name || "No business"}</small>
                    <small>{lead.interest || "General enquiry"}</small>
        ...
        {showAdd && (
          <div className="modalBackdrop">
            <form className="modal" onSubmit={addLead}>
              ...
              <small>NEW LEAD</small><h2>Add a client</h2>
        ...
        {selected && (
          <div className="drawer">
            ...
            <h2>{selected.name || selected.whatsapp_name || selected.phone}</h2>
            <p>{selected.business_name || "No business name"}</p>

            <div className="details">
              <b>Email</b><span>{selected.email || "No email added"}</span>
              <b>WhatsApp</b><span>{selected.phone}</span>
              <b>Interest</b><span>{selected.interest || "—"}</span>
              <b>Deal value</b><span>{selected.deal_value ? `₦${Number(selected.deal_value).toLocaleString()}` : "—"}</span>
              <b>Next follow-up</b><span>{selected.next_follow_up_at ? new Date(selected.next_follow_up_at).toLocaleString() : "Not scheduled"}</span>
              <b>Notes</b><span>{selected.notes || "—"}</span>
            </div>

            <div className="drawerActions">
              <button className="primaryButton full" disabled={!selected.email} onClick={() => openEmail(selected)}>{selected.email ? "✉ Email client" : "Add email to email client"}</button>
            ...
            <label>Status<select value={selected.status} onChange={(event) => updateStatus(selected.id, event.target.value)}>{stages.map((stage) => <option key={stage} value={stage}>{labels[stage]}</option>)}</select></label>
          </div>
        )}
      </main>
    </div>
