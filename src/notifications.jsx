...
function Notifications(){
 ...
 useEffect(()=>{load();const timer=setInterval(load,30000);const{data:{subscription}}=supabase.auth.onAuthStateChange(()=>load());const openHandler=()=>{setOpen(true);load()};window.addEventListener("royexa:notifications-open",openHandler);return()=>{clearInterval(timer);subscription.unsubscribe();window.removeEventListener("royexa:notifications-open",openHandler)}},[]);
 const mark=async id=>{try{const{error}=await supabase.rpc("mark_admin_notification_read",{p_message_id:id});if(error)throw error;setItems(x=>x.map(n=>n.id===id?{...n,is_read:true}:n));setUnread(x=>Math.max(0,x-1));}catch(e){setError(e?.message||"Unable to mark notification read.")}};
