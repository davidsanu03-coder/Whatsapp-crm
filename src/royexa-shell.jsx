...
function Shell({ aside }) {
  ...
  function openMessage(m) {
    clickExisting(["Messages"]);
    setTimeout(() => window.dispatchEvent(new CustomEvent("royexa:open-lead", { detail: { leadId: m.lead_id, messageId: m.id } })), 100);
  }

  return <div className={`royexa-shell ${collapsed ? "is-collapsed" : ""}`}>
    <div className="royexa-shell-brand-row">
      <button className="royexa-collapse" onClick={() => setCollapsed(v => !v)} aria-label="Toggle navigation">☰</button>
      <div className="royexa-shell-brand">ROYEXA <span>CRM</span></div>
    </div>
    <div className="royexa-shell-search">
      <span>⌕</span>
      <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search" aria-label="Search workspace" />
      <kbd>⌘K</kbd>
    </div>
    <div className="royexa-shell-scroll">
      {filteredGroups.map(group => <section className="royexa-nav-group" key={group.title}>
        <div className="royexa-nav-title">{group.title}</div>
        {group.items.map(([id, icon, label]) => <button key={id} className="royexa-nav-item" onClick={() => route(id)} title={label}><i>{icon}</i><span>{label}</span></button>)}
      </section>)}

      <section className="royexa-recent-section">
        <div className="royexa-recent-head"><span>RECENT MESSAGES</span><button onClick={loadRecent} aria-label="Refresh recent messages">↻</button></div>
        {loading ? <div className="royexa-recent-empty">Loading conversations…</div> : filteredRecent.length ? filteredRecent.map(m => <button className="royexa-recent" key={m.id} onClick={() => openMessage(m)}>
          <span className="royexa-avatar">{initials(m.name)}</span>
          <span className="royexa-recent-main"><b>{m.name}</b><small>{m.text || m.subject || "New conversation"}</small></span>
          <time>{ago(m.created_at)}</time>
        </button>) : <div className="royexa-recent-empty">{error || "No recent messages yet."}</div>}
      </section>
    </div>
    <div className="royexa-shell-footer">
      <div className="royexa-status"><span></span><b>Workspace online</b></div>
      <button className="royexa-signout" onClick={() => supabase.auth.signOut()}>Sign out</button>
    </div>
  </div>;
}
...
function mount() {
  ...
  root.id = "royexa-workspace-shell";
  aside.appendChild(root);
  createRoot(root).render(<Shell aside={aside} />);
  return true;
}
